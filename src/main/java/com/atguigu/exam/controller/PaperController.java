...
public class PaperController {
     ...
     */
    @PostMapping("/ai")
    @Operation(summary = "AI智能组卷", description = "基于设定的规则（题型分布、难度配比等）从题库随机抽题组卷")
    ...
    @Operation(summary = "更新试卷状态", description = "修改试卷状态，合法流转路径：DRAFT→PUBLISHED(启用)、PUBLISHED→STOPPED(停用)、STOPPED→PUBLISHED(重新启用)、STOPPED→DRAFT(退回编辑)")
    public Result<Void> updatePaperStatus(
            @Parameter(description = "试卷ID") @PathVariable Integer id,
            @Parameter(description = "新的状态，可选值：DRAFT/PUBLISHED/STOPPED") @RequestParam String status) {
