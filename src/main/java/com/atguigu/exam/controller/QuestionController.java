...
public class QuestionController {
    ...
    public Result<Page<Question>> getQuestionList(
            //@Parameter是Swagger的注解，用于描述参数的含义和默认值，参数描述为当前页码，从1开始，默认第1页  defaultValue = "1" 表示默认值为1页
        ...
        questionService.queryquestionListByPage(pageBean,questionQueryVo);
     ...
     * - 数据绑定：Spring使用Jackson自动将JSON转换为Java对象
     * 事务处理：
     * - 题目创建涉及多张表（题目、选项、答案）
...
// 核心业务逻辑：调用服务层方法执行题目保存操作
    ...
    public Result<Question> updateQuestion(
        ...
        // 核心业务逻辑：调用服务层方法执行题目更新
        questionService.updateQuestion(question);

        // 打印日志：记录接口调用完成及返回数据，便于问题排查与审计
        log.info("更新题目信息调用结束！回显数据为：{}", question);

        // 构建统一成功响应结果，返回更新后的题目数据（此处传入null，可改为question）
        return Result.success(question);
    }
    ...
    /**
     * 下载Excel导入模板接口
     * @return 包含Excel模板文件的HTTP响应实体，以二进制字节流形式返回
     */
// 映射HTTP GET请求，指定接口访问路径为 /template
    @GetMapping("/template")
// Swagger/OpenAPI接口文档注解：标注接口名称与详细描述，用于生成接口文档
    @Operation(summary = "下载Excel导入模板", description = "下载题目批量导入的Excel模板文件")
// 接口方法定义：
// 返回值 ResponseEntity<byte[]>：完整的HTTP响应对象，泛型为字节数组，用于承载Excel文件的二进制数据
// 抛出IOException：声明文件操作过程中可能出现的输入输出异常，交由框架统一处理
    public ResponseEntity<byte[]> downloadTemplate() throws IOException {
        // 1. 调用Excel工具类，生成标准导入模板的Excel文件，将文件转换为内存中的字节数组
        byte[] template = ExcelUtil.generateTemplate();
        // 新增这行，关键排查点
        System.out.println("生成的Excel字节长度：" + template.length);

        // 2. 构建完整的HTTP响应对象，封装文件数据与响应头信息
        ResponseEntity<byte[]> responseEntity = ResponseEntity.ok() // 设置HTTP响应状态码为200，代表请求成功
                // 设置响应头Content-Disposition：告诉浏览器该文件为附件，触发下载行为，指定下载后的文件名为template.xlsx
                .header("content-disposition", "attachment;filename=template.xlsx")
                // 设置响应体的Content-Type：声明内容为二进制流文件，浏览器不会尝试直接打开，只会执行下载
                .contentType(MediaType.APPLICATION_OCTET_STREAM)
                // 设置响应体核心内容：就是生成好的Excel模板字节数组
                .body(template);

        // 3. 返回构建完成的响应对象，SpringMVC会自动将其解析为标准HTTP响应返回给前端/浏览器
